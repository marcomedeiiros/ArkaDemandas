...
const PRIORITY_CONFIG = {
  Baixa:   { color: '#4ADE80', emoji: '🟢', bg: 'rgba(34,197,94,0.12)',  border: 'rgba(34,197,94,0.3)'  },
  Média:   { color: '#FCD34D', emoji: '🟡', bg: 'rgba(234,179,8,0.12)',  border: 'rgba(234,179,8,0.3)'  },
  Alta:    { color: '#FB923C', emoji: '🟠', bg: 'rgba(249,115,22,0.12)', border: 'rgba(249,115,22,0.3)' },
  Urgente: { color: '#F87171', emoji: '🔴', bg: 'rgba(239,68,68,0.12)',  border: 'rgba(239,68,68,0.3)'  },
};
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 pt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Field label="Título"      icon="🖥"  value={form.titulo}      onChange={v => update('titulo', v)}      required placeholder="Título da demanda" />
            <Field label="Cliente"     icon="🏢"  value={form.cliente}     onChange={v => update('cliente', v)}     required placeholder="Nome do cliente" />
            <Field label="Responsável" icon="👤"  value={form.responsavel} onChange={v => update('responsavel', v)} required placeholder="Técnico responsável" />
            <Field label="Categoria"   icon="📂"  value={form.categoria}   onChange={v => update('categoria', v)}   required placeholder="Categoria da demanda" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {/* Prazo */}
            <Field label="Prazo" icon="⏰" value={form.prazo ?? ''} onChange={v => update('prazo', v)} type="date" />
          </div>
